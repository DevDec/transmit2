...
int main() {
    ...
    char privkey_path[256], pubkey_path[256], password[256];
    char input[512];
    ...
    if (strcmp(auth_method, "password") == 0) {
        ...
        // Derive public key path by appending .pub
        snprintf(pubkey_path, sizeof(pubkey_path), "%s.pub", privkey_path);

        if (init_sftp_session(hostname, username, privkey_path, pubkey_path, &sftp_session, &session, &sock) != 0) {
