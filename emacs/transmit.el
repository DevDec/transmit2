...
;; bin/ directory at the root of the transmit2 repo:
...
When nil the binary is found automatically by searching the straight.el
repos directory and then the directory of this file.
Example: (setq transmit-binary-path \"/path/to/transmit2/bin/transmit-linux\")"
  ...
  (condition-case err
      (if (file-exists-p transmit-data-file)
        ...
        (let ((tbl (make-hash-table :test 'equal)))
          (transmit--write-data tbl) tbl))
    (error (transmit--log 4 (format "Failed to read transmit.json: %s" err) t) nil)))
...
(defun transmit--write-data (data)
  ...
  (condition-case err
      (progn
        ...
        t)
    (error (transmit--log 4 (format "Failed to write transmit.json: %s" err) t) nil)))
  ...
  (cond
   ...
   ((file-exists-p path)
    ...
    (if (file-executable-p path) path
      (transmit--log 4 (format "Cannot make executable: %s" path) t) nil))
   (t nil)))
    ...
    (let ((id transmit--next-queue-id))
      ...
      (setq transmit--queue
            (nconc transmit--queue
                   (list (list :id id :type type :filename filename
                               :working-dir working-dir :processing nil))))
      (transmit--log 1 (format "Queued [%d]: %s %s" id type filename))
      (transmit--ensure-connection #'transmit--process-next)
      id)))

(defun transmit--queue-head () (car transmit--queue))
(defun transmit--dequeue () (setq transmit--queue (cdr transmit--queue)))

(defun transmit--find-queue-item (id)
  "Return (item . index) for queue item ID, or nil."
  (cl-loop for item in transmit--queue for i from 0
           when (= (plist-get item :id) id) return (cons item i)))
        ...
        (run-with-timer transmit-keepalive-timeout nil
          (lambda ()
            (when transmit--process
              ...
              (setq transmit--process nil transmit--phase transmit--phase-init
                    transmit--connecting nil transmit--connection-ready nil)
...
(defun transmit--filter (proc string)
  "Process output STRING from PROC, driving the state machine."
...
(defun transmit--handle-line (proc line cfg)
  "Route LINE through the connection state machine."
    ...
    (cond
     ...
     ((and (string= transmit--phase transmit--phase-auth-method)
      ...
      (let ((auth-type (or (gethash "auth_type" creds) "key")))
        ...
        (setq transmit--phase
              (if (string= auth-type "password") transmit--phase-password
                transmit--phase-key))))
...
(defun transmit--sentinel (_proc event)
  ...
  (unless transmit--is-exiting
    ...
    (dolist (item transmit--queue) (plist-put item :processing nil))
    (transmit--ensure-connection #'transmit--process-next)))
...
(defun transmit--excluded-p (path)
  ...
  (cl-some (lambda (pat) (string-match-p pat path)) transmit--excluded-patterns))
...
(defun transmit--find-watch-root (file)
  ...
  (cl-loop for root being the hash-keys of transmit--watchers
           when (string-prefix-p root file) return root))
    ...
    (let ((subdirs (transmit--list-subdirs root))
      ...
      (dolist (dir subdirs)
        (unless (or (transmit--excluded-p dir) (gethash dir tbl))
          (condition-case err
              (progn
                (puthash dir (file-notify-add-watch dir '(change) #'transmit--watch-callback) tbl)
                (cl-incf count))
...
(defun transmit--list-subdirs (root)
  "Return ROOT and all sub-directories, excluding excluded paths."
  (let (result)
...
(defun transmit--on-kill-emacs ()
  ...
  (when transmit--process
    (condition-case nil (process-send-string transmit--process "exit\n") (error nil))
    (delete-process transmit--process)
    (setq transmit--process nil))
  (when transmit--keepalive-timer    (cancel-timer transmit--keepalive-timer))
  (when transmit--auth-timeout-timer (cancel-timer transmit--auth-timeout-timer))
  ...
  (let ((servers (transmit--server-names)))
    ...
    (let* ((choice (completing-read
      ...
      (if (string= choice "none")
          (progn (transmit--update-selection "none" nil cwd)
        ...
        (let ((remotes (transmit--remote-names choice)))
          ...
          (let ((remote (completing-read (format "Remote for %s: " choice) remotes nil t)))
            ...
            (let ((cfg (gethash choice transmit--server-config)))
              (when (and cfg (gethash "upload_on_bufwrite" cfg)) (transmit--install-auto-upload))
              (when (and cfg (gethash "watch_for_changes" cfg))  (transmit-watch-directory)))))))))
...
(defun transmit-disconnect ()
  ...
  (interactive)
  (if transmit--process
      (progn (process-send-string transmit--process "exit\n")
    ...
    (if (null found)
      ...
      (let ((item (car found)))
        (if (plist-get item :processing)
          ...
          (setq transmit--queue
                (cl-remove-if (lambda (i) (= (plist-get i :id) id)) transmit--queue))
