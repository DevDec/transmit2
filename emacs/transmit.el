...
;; M-x transmit-upload-modified - Upload all git-modified files
...
;; M-x transmit-reset - Hard reset queue and connection (unstick)

;; M-x transmit-retry - Retry all queued items (keep files)
...
;; Watchdog stuck-item timeout in seconds. Items processing longer than this
;; are assumed stuck and are retried, even if the process is still alive.
(defcustom transmit-stuck-timeout 30

  "Seconds after which a processing queue item is considered stuck."
         ...
         (stuck
          ...
          (transmit--log 2
                         ...
                         (if process-dead
                             ...
                             "Watchdog: unsticking (process died)"

                           (format "Watchdog: unsticking (stalled %.0fs)" elapsed)))

          (plist-put head :processing nil)

          (plist-put head :started-at nil)
        ...
        (let* ((cwd (plist-get item :working-dir))
          ...
          (let* ((relative (file-relative-name filename root))
            ...
            (when cmd
              ...
              (plist-put item :started-at (float-time))  ; ← timestamp for watchdog
  ...
  (when (and buffer-file-name
    ...
    (unwind-protect
        ...
        (let ((file (expand-file-name buffer-file-name))
          ...
          (transmit--upload file root))

      (setq transmit--save-in-progress nil))))

(defun transmit--install-auto-upload-hook ()

  "Install the after-save hook for auto-upload (idempotent)."
...
(defun transmit-select-server ()
  ...
  "Interactively select a server and remote for the current project."

  (interactive)

  (let ((servers (hash-table-keys transmit--server-config)))

    (unless servers

      (user-error "No servers configured — call transmit-setup first"))

    (let* ((server-name (completing-read "Select server: " servers nil t))

           (cfg (gethash server-name transmit--server-config))

           (remotes (and cfg (hash-table-keys (or (gethash "remotes" cfg)

                                                   (make-hash-table)))))

           (all-choices (append remotes '("none")))

           (remote (completing-read

                    (format "Remote for %s: " server-name) all-choices nil t)))

      (if (string= remote "none")

          (progn

            (transmit--update-selection "none" nil)

            (message "Transmit: server selection cleared"))

        (transmit--update-selection server-name remote)

        (transmit--install-auto-upload-hook)

        (message "Transmit: using %s → %s" server-name remote)))))

;;;###autoload

(defun transmit-upload-file ()

  "Upload the current buffer's file."
  ...
  (let ((id (transmit--upload buffer-file-name default-directory)))

    (when id

      (message "Transmit: queued upload [%d] %s"
...
(defun transmit-upload-modified ()
  ...
  "Upload all files reported as modified by git in the current project."
  ...
  (let* ((root (transmit--project-root))
         ...
         (output (shell-command-to-string "git diff --name-only HEAD"))

         (untracked (shell-command-to-string "git ls-files --others --exclude-standard"))

         (all-lines (append (split-string output "\n" t)

                            (split-string untracked "\n" t)))

         (files (cl-remove-if #'string-empty-p all-lines))

         (count 0))

    (if (null files)

        (message "Transmit: no modified or untracked files found by git")
      ...
      (dolist (rel files)
        ...
        (let ((abs (expand-file-name rel root)))
          ...
          (when (file-regular-p abs)
            ...
            (transmit--upload abs root)

            (cl-incf count))))

      (message "Transmit: queued %d modified/untracked file(s)" count))))

;;;###autoload

(defun transmit-remove-file ()

  "Remove the current buffer's file from the remote."
  ...
  (let ((id (transmit--remove buffer-file-name default-directory)))

    (when id

      (message "Transmit: queued remove [%d] %s"

               id (file-name-nondirectory buffer-file-name)))))

;;;###autoload

(defun transmit-watch-directory ()

  "Watch the current project root for changes and auto-upload."
  ...
  (let ((root (transmit--project-root)))
    ...
    (unless (transmit--working-dir-has-selection-p root)
      ...
      (user-error "No server selected for project %s — run transmit-select-server first"
...
(defun transmit-stop-watching ()

  "Stop all file watchers."

  (interactive)

  (let ((n (transmit--stop-watching)))

    (message "Transmit: removed %d watcher(s)" n)))

;;;###autoload

(defun transmit-show-queue ()

  "Show the upload queue buffer."
...
(defun transmit-clear-queue ()
  ...
  "Clear all pending (non-processing) items from the queue."

  (interactive)

  (setq transmit--queue

        (cl-remove-if-not (lambda (i) (plist-get i :processing)) transmit--queue))

  (transmit--modeline-refresh)

  (transmit--maybe-refresh-queue-buffer)

  (message "Transmit: queue cleared"))

;;;###autoload

(defun transmit-reset ()

  "Hard-reset the queue and connection state. Use when stuck.

Marks all queue items as not-processing so they will be retried on reconnect."
  ...
  (message "Transmit: reset complete — %d item(s) remain in queue"
...
(defun transmit-retry ()
  ...
  "Retry all queued items without clearing them.

Resets connection state and kicks off a fresh upload cycle."
  ...
  (if transmit--queue
      ...
      (progn
        ...
        (message "Transmit: retrying %d item(s)" (length transmit--queue)))

    (message "Transmit: queue is empty — nothing to retry")))

;;;###autoload

(defun transmit-show-log ()

  "Show the transmit debug log buffer."

  (interactive)

  (let ((buf (get-buffer-create "*transmit-log*")))
...
(defun transmit-disconnect ()
  ...
  (message "Transmit: disconnected"))

;;;###autoload

(defun transmit-status ()

  "Show a summary of the current connection and queue in the echo area."
  ...
  (let* ((server (transmit--get-selected-server))
         ...
         (q-len (length transmit--queue))

         (processing (cl-count-if (lambda (i) (plist-get i :processing))

                                   transmit--queue))

         (state (cond (transmit--connection-ready "connected")

                      (transmit--connecting       "connecting")

                      (t                          "disconnected"))))

    (message "Transmit: server=%s  remote=%s  state=%s  queue=%d  active=%d"
...
(defun transmit-get-progress ()
  ...
  "Return the current upload progress as a plist with :file and :percent."
...
(defun transmit-setup (config-file)

  "Load CONFIG-FILE (JSON), populate server config, and activate the modeline.

CONFIG-FILE should be a JSON object mapping server names to server configs.

Each server config should have:
  credentials: { host, username, auth_type, identity_file or password }
  remotes: { remote-name: \"/remote/base/path\", ... }"

  (interactive "fTransmit config file: ")

  (condition-case err

      (let* ((expanded (expand-file-name config-file))
        ...
        (maphash (lambda (name cfg)

                   (puthash name cfg transmit--server-config))

                 data)

        (transmit--setup-modeline)

        ;; Restore last-used server/remote from saved data

        (let* ((saved (transmit--read-data))

               (any (and saved (transmit--find-any-selection saved))))
        ...
        (transmit--log 2 (format "Loaded %d server(s) from %s"
    ...
    (error
     ...
     (transmit--log 4 (format "transmit-setup failed: %s" err) t))))
